...
contract GenericHandlerUpgradeable is Initializable, AccessControlUpgradeable {
    ...
    function replayGenericDeposit(
        ...
        FeeRecord storage feeRecord = _feeRecord[_destChainID][_depositNonce];
        uint256 preGasPrice = feeRecord._gasPrice;
        uint256 preGasLimit = feeRecord._gasLimit;

        require(
            preGasLimit <= _gasLimit,
            "GenericHandler: Gas Limit on replay must be equal to greater than previous GasLimit"
        );

        require(preGasPrice < _gasPrice, "GenericHandler: Gas Price on replay must be greater than previous Price");

        uint256 fee = deductFee(_destChainID, record._feeToken, _gasLimit, _gasPrice, true);

        _feeRecord[_destChainID][_depositNonce]._gasLimit = _gasLimit;
        _feeRecord[_destChainID][_depositNonce]._gasPrice = _gasPrice;
    ...
    /// @param _chainID Array of ChainIDs of the destination chain.
    /// @param _defaultGas Array of Default gas for the chainid.
    function setDefaultGas(uint8[] memory _chainID, uint256[] memory _defaultGas) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_chainID.length == _defaultGas.length, "GenericHandler: Array length mismatch");
    ...
    {
        require(_chainID.length == _defaultGasPrice.length, "GenericHandler: Array length mismatch");
    ...
    ///@notice Function Sets chainId for the chain.
    ///@param _chainID ChainID for the contract.
    function setChainId(uint8 _chainID) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _chainId = _chainID;
    }
    ...
    /// @param feeFactorX10e6 FeeFactor for the cross chain call.
    /// @param bridgeFee Base Fee for bridge.
    /// @param accepted Bool value for enabling and disabling feetoken.
    function setFees(
        uint8 destinationChainID,
        address feeTokenAddress,
        uint256 feeFactorX10e6,
        uint256 bridgeFee,
        bool accepted
    ) external onlyRole(FEE_SETTER_ROLE) {
        feeManager.setFee(destinationChainID, feeTokenAddress, feeFactorX10e6, bridgeFee, accepted);
    }
    ...
    function deductFee(
        ...
        if (feeTokenDecimals < 18) {
            uint8 decimalsToDivide = 18 - feeTokenDecimals;
            fees = bridgeFees + ((feeFactorX10e6 * gasPrice * gasLimit) / (10**(6 + decimalsToDivide)));
        } else {
