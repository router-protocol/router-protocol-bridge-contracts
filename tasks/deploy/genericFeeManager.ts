...
task("Deploy:GenericFeeManager").setAction(async function (_taskArguments: TaskArguments, hre) {
  const deployContract = "FeeManagerGenericUpgradeable";
  const network = await hre.getChainId();
  const handlerAddr = deployments[network].GenericHandlerUpgradeable;

  console.log("Contract Deployment Started ");
  const C1 = await hre.ethers.getContractFactory("FeeManagerGenericUpgradeable");
