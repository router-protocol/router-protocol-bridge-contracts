...
task("Deploy:UsdcMinter").setAction(async function (_taskArguments: TaskArguments, hre) {
  const deployContract = "UsdcMinter";
  ...
  const C1 = await hre.ethers.getContractFactory("UsdcMinter");
  const c1Proxy = await hre.upgrades.deployProxy(
    C1,
    [
      usdcConfigs[network].Usdc,
      usdcConfigs[network].MessageTransmitter,
