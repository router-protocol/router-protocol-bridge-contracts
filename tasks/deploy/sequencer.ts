...
task("Deploy:Sequencer").setAction(async function (_taskArguments: TaskArguments, hre) {
  const deployContract = "SequencerHandlerUpgradeable";
  const network = await hre.getChainId();

  console.log("Contract Deployment Started ");
  const C1 = await hre.ethers.getContractFactory("SequencerHandlerUpgradeable");
  const c1Proxy = await hre.upgrades.deployProxy(C1, [deployments[network].BridgeUpgradeable.proxy]);
  console.log(c1Proxy);
  ...
  console.log(c1Proxy.address,"-",implementationAddr)
