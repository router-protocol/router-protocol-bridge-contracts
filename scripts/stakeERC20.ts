...
async function testScript() {
  ...
  //   DestinationVoterInstance = (await upgrades.deployProxy(VoterContract, [], { kind: "uups" })) as VoterUpgradeable;
    ...
    originResourceID = createResourceID(newtestTokenDeployed["42"].TestToken.newtoken, 0);
    ...
    tx = await OriginBridgeInstance.adminSetLiquidityPool(
      "RDAI",
      "RDAI",
      18,
    ...
    tx = await OriginBridgeInstance.connect(depositerAddress).stake(
      originResourceID,
      OriginERC20MintableInstance.address,
