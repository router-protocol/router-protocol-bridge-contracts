...
async function testTokenConfigure() {
  console.log("------------------------------ Initial Setup Started ------------------------------");
  let deployContract = "TestToken";

  const hre = require("hardhat");
  let deployedContracts = require("../deployment/reserveToken.json");
  ...
  // const Voter = await hre.ethers.getContractFactory("VoterUpgradeable");
  const C1Addr = C1.attach(deployedContracts[network].TestToken.token1);
  ...
  // const VoterAddr = Voter.attach(voter[network].VoterUpgradeable.proxy);
  ...
  const dfynResourceId = createResourceID(deployedContracts["42"].TestToken.token1, 0);
  const routeResourceId = createResourceID(deployedContracts["42"].TestToken.token2, 0);

  console.log("ETH resource id : "+ethResourceId);
  console.log("usdt resource id : "+dfynResourceId);
  console.log("usdc resource id : "+routeResourceId);
  ...
  await tx.wait(2);
  tx = await BridgeAddr.grantRole(relayerRole, "0x33bCDe3dA095C1D2681F3167E5696bf5b7d4F321");
  await tx.wait(2);
  tx = await BridgeAddr.grantRole(relayerRole, (await ethers.getSigners())[1].address);
  await tx.wait(2);
  tx = await BridgeAddr.adminSetResource(handler[network].ERC20HandlerUpgradeable.proxy, ethResourceId, ETH);
  await tx.wait(2);
  tx = await BridgeAddr.adminSetResource(handler[network].ERC20HandlerUpgradeable.proxy, dfynResourceId, C1Addr.address);
  await tx.wait(2);
  ...
  await tx.wait(2);
  tx = await BridgeAddr.adminSetFeeStatus(dfynResourceId, true);
  await tx.wait(2);
  ...
  await tx.wait(2);
  tx = await BridgeAddr.setBridgeFee(dfynResourceId, destChain, C1Addr.address, 0, 0, true);
  await tx.wait(2);
  ...
  tx = await WETHInstance.deposit({ value: "100000000000000000" });
  await tx.wait(2);
  tx = await WETHInstance.transfer(reserve[network].HandlerReserveUpgradeable.proxy, "100000000000000000");
  await tx.wait(2);
  ...
  await tx.wait(2);
  // let tx = await C1Addr.mint("0x33bCDe3dA095C1D2681F3167E5696bf5b7d4F321", "10000000000000000000000");
  // await tx.wait(2);
  // tx = await C2Addr.mint("0x33bCDe3dA095C1D2681F3167E5696bf5b7d4F321", "10000000000000000000000");
  // await tx.wait(2);
  console.log(" Depositor balance = " + (await C1Addr.balanceOf((await ethers.getSigners())[0].address)).toString() + " DFYN");
  console.log(" Depositor balance = " + (await C2Addr.balanceOf((await ethers.getSigners())[0].address)).toString() + " ROUTE");
  // console.log(" totalsupply = " + (await VoterAddr.balanceOf("0x33bCDe3dA095C1D2681F3167E5696bf5b7d4F321")).toString());
