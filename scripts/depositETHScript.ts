...
async function testScript() {
  ...
  //   DestinationVoterInstance = (await upgrades.deployProxy(VoterContract, [], { kind: "uups" })) as VoterUpgradeable;
    ...
    // OriginERC20MintableInstance = (await ERC20MintPause.attach((testTokenDeployed[await getChainId()]).TestToken.proxy)) as RouterERC20Upgradable;
    ...
    originResourceID = createResourceID(ETH, 0);
    ...
    //APPROVAL
    // await OriginERC20MintableInstance.approve(reserve[await getChainId()].HandlerReserveUpgradeable.proxy, "10000000");
